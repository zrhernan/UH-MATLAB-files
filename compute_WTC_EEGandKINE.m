...
rmpath(genpath('C:\Users\zrhernan\Documents\MATLAB\fieldtrip\realtime'))
...
for ii=6%:length(InfantID)
    ...
    fullFileName1 = 'EEG\trialRemovedEEG_kinematicsBased.mat';
...
%         data.sampleinfo(2) = length(data.time{1});
        ...
        eegset = dataClean_outlierTrlsRmvd; clear dataClean_outlierTrlsRmvd;
...
%         eegset = dataRejectHighZchn; clear dataRejectHighZchn;
        eegset.sampleinfo = eegset.cfg.previous.previous.trl(:,1:2);%trl(:,1:2);
        eegset.cfg.conditionlabel = eegset.cfg.previous.previous.conditionlabel;%conditionlabel;
    end
    
    %% Load Kinematics FT dataset (w/trials)   
    % Check if the kinematics file exists
    fullFileName2 = 'Kinematics\trialRemovedACCEL_kinematicsBased.mat';
...
%         datakine_filt.sampleinfo(2) = length(datakine_filt.time{1});
        load(fullFileName2);
        accelset = datakineTrlresamp_outlierTrlsRmvd; clear datakineTrlresamp_outlierTrlsRmvd datakineTrlresamp;
        accelset.cfg.conditionlabel = accelset.cfg.previous.conditionlabel;
    end
...
%
    ...
    ts = eegset.time{1}; % extract time samples that correspond to the data
    headsensidcs = find(strncmp(accelset.label,'InfantsForehead',15)==1);
    if isempty(headsensidcs),disp('No IMU Head Acceleration found. Moving to next infant.'),continue, end
    SelectedChannelList = {'Fz','CPz','Oz'};
    ClassOrderList = {'rest','explore','observe','reach-grasp','reach-offer','imitate'};

    clscntr = 0;
    for cls = 1:length(eeg_dClass)
        classname = lower(eeg_dClass{cls}.conditionlabel);
        chncntr = 0;
        MagnAccelset = zeros(length(kine_dClass{cls}.trial),length(ts));
        numtrls = length(kine_dClass{cls}.trial);
        for trl = 1:numtrls,
            MagnAccelset(trl,:) = sqrt(sum(kine_dClass{cls}.trial{trl}(headsensidcs,:)'.^2, 2));
        end
        MuMagnAccelset = mean(MagnAccelset,1);
        MuEEGset = mean(cat(3,eeg_dClass{cls}.trial{:}),3);
        if all(isempty(MuEEGset)) || all(isnan(MuMagnAccelset))|| all(MuMagnAccelset==0), continue, end
        clscntr = clscntr + 1;
        ...
        for chn = 1:length(eegset.label)
            ...
            if any(strcmp(channame,SelectedChannelList))==1
                ...
                ds1 = zscore(MuEEGset(chn,:));
                ds2 = zscore(MuMagnAccelset);
                options = {'mcc',100,'S0',0.1};%,'ms',5};
    %             if strcmp(InfantID{ii},'CM17'), options = {options,'AR1','burg' };
                [Rsq,period,~,coi,wtcsig,Wxy] = wtcwPARFOR([ts;ds1]',[ts;ds2]',options{:}); % to speed up Monte Carlo test

                figure(1), set(gcf,'color',[1 1 1])
                subplot(6,3,sub2ind([3,6],find(ismember(SelectedChannelList,channame)==1),find(ismember(ClassOrderList,classname)==1)))
                makeWTCfigure(Rsq,period,coi,wtcsig,Wxy,ts,options)
                if find(ismember(ClassOrderList,classname)==1) == 1, title([channame ' & |ACCEL_H_E_A_D|']), end
%                 if strcmp('Oz',channame), colorbar, end
                if strcmp('Fz',channame), ylabel([upper(classname),10,'(N=',num2str(numtrls),')',10,10,'Frequency (Hz)']), end
                if find(ismember(ClassOrderList,classname)==1) == length(eeg_dClass), xlabel('Time (sec)'), end
            else
                continue
            end
        end
...
%     cbhndl = colorbar;
%     ylabel(cbhndl,'Magnitude Coherence','fontsize',12)
...
%}
    ...
    savefig('wtcohere_meanEEGchans_headaccel.fig')
    cd('\\bmi-nas-01\Contreras-UH\Infantdata\Infantdata\wavelet coherence plots removed outlying trials')
    print(myfig,'-dpng', [InfantID{ii},'_',filename], '-r600')
%     saveppt2(['MNS_WTC_analysis_',num2str(ii),'.ppt'],'title',['Wavelet Coherence [0.5 50] Hz for Subject ',InfantID{ii}],'stretch','off','res',220);
    close all
